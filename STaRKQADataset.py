...
class STaRKQADataset(InMemoryDataset):
    ...
    def __init__(
        ...
        reltype_path = os.path.join(base_emb, 'doc/reltype_emb_dict.pt')
        triplet_path = os.path.join(base_emb, 'doc/triplet_sentence_emb_dict.pt')

        # Load optionally — these files may not be present (only needed when
        # edge_embedding_method is 'reltype' or 'triplet').
        # When absent the code falls back to zero edge prizes (node prizes only).
        self.reltype_embedding_dict = (
            torch.load(reltype_path, weights_only=False)
            if os.path.exists(reltype_path) else None
        )
        self.triplet_embedding_dict = (
            torch.load(triplet_path, weights_only=False)
            if os.path.exists(triplet_path) else None
        )
        self.query_embedding_dict = torch.load(
            os.path.join(base_emb, 'query/query_emb_dict.pt'),
            weights_only=False,
        )
    ...
    def process(self) -> None:
        ...
        if os.path.exists(base_subgraph_file):
            base_subgraph_rels = torch.load(base_subgraph_file, weights_only=False)
            print(f"Loading of precomputed base subgraphs completed in {time.time() - t:.2f}s")
        else:
            ...
            print(f"All cypher query retrieval completed in {time.time() - t:.2f}s")
        ...
        for index, qa_row in tqdm(dataframe.iterrows()):
            ...
            # Embed edges for prize assignment
            edge_method = pcst_config.get("edge_embedding_method", "none")
            if edge_method == 'triplet' and self.triplet_embedding_dict is not None:
                subgraph_rels['textEmbedding'] = self._embed_triplet(
                    subgraph_rels['srcType'], subgraph_rels['relType'], subgraph_rels['tgtType'])
            elif edge_method != 'none' and self.reltype_embedding_dict is not None:
                subgraph_rels['textEmbedding'] = self._embed(subgraph_rels['relType'])
            else:
                # No edge embeddings available — use zero vectors so edge prizes are 0
                # (only node prizes will drive PCST selection)
                dim = next(iter(self.query_embedding_dict.values())).shape[-1]
                subgraph_rels['textEmbedding'] = [np.zeros(dim)] * len(subgraph_rels)
            ...
            if self.algo_config_version in [42, 43]:
                ...
                node_prizes, edge_prizes = assign_prizes_modified(
                    pcst_base_graph_topology, mapped_topn_node_ids, top_edges, second_top_edges,
                    pcst_config["top_edge_prize"], pcst_config["second_edge_prize"])
            else:
                with GraphDatabase.driver(NEO4J_URI, auth=(NEO4J_USERNAME, NEO4J_PASSWORD)) as driver:
                ...
                k_edges = pcst_config.get("k_edges", 4)
            ...
            pcst, inner_id_mapping, selected_nodes, selected_edges = compute_pcst(
                pcst_base_graph_topology, node_prizes, edge_prizes)
        ...
        print(f"All PCST completed in {time.time() - t:.2f}s")
    ...
    def get_edges_by_reltype_vector_search(self, qa_row_id: int, subgraph_rels: DataFrame):
        ...
        grouped['cosine_sim'] = grouped['textEmbedding'].apply(
            lambda emb: cosine_similarity(query_embedding, emb.reshape(1, -1))[0][0])
        ...
        top_edges = subgraph_rels[
            subgraph_rels[['srcType', 'relType', 'tgtType']].apply(tuple, axis=1) == tuple(first_group)
        ].index.to_numpy()

        if len(top_2_groups) > 1:
            second_group = top_2_groups.iloc[1]
            second_edges = subgraph_rels[
                subgraph_rels[['srcType', 'relType', 'tgtType']].apply(tuple, axis=1) == tuple(second_group)
            ].index.to_numpy()
        else:
    ...
    def _embed_triplet(self, src_type_list: List[str], rel_type_list: List[str], tgt_type_list: List[str]):
        return [self.triplet_embedding_dict[(src_type, rel_type, tgt_type)]['embedding']
                for src_type, rel_type, tgt_type in zip(src_type_list, rel_type_list, tgt_type_list)]
