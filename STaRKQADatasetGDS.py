...
class STaRKQADataset(InMemoryDataset):
    ...
    def process(self) -> None:
        ...
        for index, (question_id, prompt, _) in tqdm(dataframe.iterrows()):
            ...
            # Run the pcst algorithm via Neo4j GDS
            ...
            # Take union with top-K vector search nodes
            pcst_nodes = np.unique(np.concatenate((pcst_nodes, topk_nodes)))

            # Retrieve node embeddings, labels and textual graph description
            with GraphDatabase.driver(NEO4J_URI, auth=(NEO4J_USERNAME, NEO4J_PASSWORD)) as driver:
            ...
            all_pcst_nodes[index] = textual_nodes_df['nodeId'].tolist()
            ...
            edge_index = torch.tensor(
                [(consecutive_map[src], consecutive_map[tgt]) for src, tgt in pcst_edges],
                dtype=torch.int32,
            ).T
            enriched_data = Data(
                x=node_embedding,
